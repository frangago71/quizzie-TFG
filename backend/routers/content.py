...
def post_quiz(
    ...
    for q in quiz_data.questions:
        db_question = Question(text=q.text, points=q.points, quiz=db_quiz)
        session.add(db_question)
        for o in q.options:
            db_option = Option(text=o.text, is_correct=o.is_correct, question=db_question)
...
def update_quiz(
    ...
    if len(quiz_data.questions) == 0:
        raise HTTPException(
            status_code=400, detail="El cuestionario debe tener al menos una pregunta."
        )
...
def delete_quiz_and_rooms(
    ...
    if active_rooms:
        raise HTTPException(
            status_code=400,
            detail=("No se puede borrar el cuestionario porque tiene una sala activa."),
        )
...
def create_question(
    ...
    new_question = Question(text=question_data.text, points=question_data.points, quiz_id=quiz_id)
    session.add(new_question)
    session.commit()
...
def delete_option(
    ...
    if db_option.is_correct:
        ...
        if len(correct_options) <= 1:
            raise HTTPException(
                status_code=400,
                detail="No puedes borrar la única opción correcta de una pregunta.",
            )
