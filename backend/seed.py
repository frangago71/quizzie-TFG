...
def _seed_groups(session: Session, t1, t2):
    # --- 2. GRUPOS ---
    ...
    return g_cervantes, g_cajal


def _seed_students(session: Session, g_cervantes, g_cajal):
    # --- 3. ESTUDIANTES ---
    all_students = []
...
def _seed_quizzes(session: Session, t1, t2, t3, t4):
    # --- 4. CUESTIONARIOS ---
    quiz_lengua = Quiz(
...
def _seed_rooms(
    ...
    for r in [r_waiting1, r_waiting2, r_live, r_finished, r_verifying]:
        session.refresh(r)
    return r_waiting1, r_waiting2, r_live, r_finished, r_verifying


def _seed_participants(
    session: Session, all_students, r_waiting1, r_waiting2, r_live, r_finished, r_verifying
):
    # --- 8. PARTICIPANTES ---
    for i in range(0, 40):
        session.add(
...
def create_seed_data():
    ...
    with Session(engine) as session:
        ...
        t1, t2, t3, t4 = _seed_teachers(session, password)
        g_cervantes, g_cajal = _seed_groups(session, t1, t2)
        all_students = _seed_students(session, g_cervantes, g_cajal)
        ...
        (
            ...
            quiz_lengua_sin_sala_2,
            quiz_lengua_sin_sala_3,
        ) = _seed_quizzes(session, t1, t2, t3, t4)
        ...
        r_waiting1, r_waiting2, r_live, r_finished, r_verifying = _seed_rooms(
            session,
            t1,
            t2,
            t3,
