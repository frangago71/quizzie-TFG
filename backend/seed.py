...
def create_seed_data():
    ...
    with Session(engine) as session:
        ...
        session.commit()
        session.refresh(g_cervantes); session.refresh(g_cajal)
        ...
        session.commit()
        for s in all_students: session.refresh(s)

        # --- 4. CUESTIONARIOS ---
        quiz_lengua = Quiz(title="Lengua Española Básica", description="Ortografía y gramática", teacher_id=t1.id)
        quiz_ciencias = Quiz(title="Ciencias de la Naturaleza", description="Nivel primaria", teacher_id=t2.id)
        quiz_deporte = Quiz(title="Deportes y Reglas", description="Iniciación deportiva", teacher_id=t3.id)
        quiz_informatica = Quiz(title="Informática Básica", description="Hardware y Software", teacher_id=t4.id)
        quiz_lengua_sin_sala_1 = Quiz(title="Lengua Española Avanzada", description="Literatura y análisis sintáctico", teacher_id=t1.id)
        quiz_lengua_sin_sala_2 = Quiz(title="Literatura Universal", description="Obras y autores clásicos", teacher_id=t1.id)
        quiz_lengua_sin_sala_3 = Quiz(title="Gramática Española", description="Reglas gramaticales y sintaxis", teacher_id=t1.id)
        
        session.add_all([quiz_lengua, quiz_ciencias, quiz_deporte, quiz_informatica, quiz_lengua_sin_sala_1, quiz_lengua_sin_sala_2, quiz_lengua_sin_sala_3])
        session.commit()
        for qz in [quiz_lengua, quiz_ciencias, quiz_deporte, quiz_informatica, quiz_lengua_sin_sala_1, quiz_lengua_sin_sala_2, quiz_lengua_sin_sala_3]: session.refresh(qz)
        ...
        # Ciencias
        qc1 = Question(text="¿Qué gas necesitamos para respirar?", points=5, quiz_id=quiz_ciencias.id)
        qc2 = Question(text="¿Cuál es el planeta más cercano al Sol?", points=5, quiz_id=quiz_ciencias.id)
        # Deporte
        qd1 = Question(text="¿Cuántos jugadores hay en un equipo de fútbol?", points=5, quiz_id=quiz_deporte.id)
        qd2 = Question(text="¿En qué deporte se utiliza una raqueta?", points=5, quiz_id=quiz_deporte.id)
        # Informática
        qi1= Question(text="¿Cuál es un sistema operativo?", points=5, quiz_id=quiz_informatica.id)
        qi2 = Question(text="¿Qué componente es el 'cerebro' del ordenador?", points=5, quiz_id=quiz_informatica.id)
        # Lengua sin sala (1)
        qlss1_1 = Question(text="¿Cuál es el sujeto en la frase 'El perro corre rápido'?", points=5, quiz_id=quiz_lengua_sin_sala_1.id)
        qlss1_2 = Question(text="¿Cuál es el predicado en la frase 'El perro corre rápido'?", points=5, quiz_id=quiz_lengua_sin_sala_1.id)
        # Lengua sin sala (2)
        qlss2_1 = Question(text="¿Quién escribió 'Don Quijote de la Mancha'?", points=5, quiz_id=quiz_lengua_sin_sala_2.id)
        qlss2_2 = Question(text="¿En qué siglo se publicó 'Don Quijote de la Mancha'?", points=5, quiz_id=quiz_lengua_sin_sala_2.id)
        # Lengua sin sala (3)
        qlss3_1 = Question(text="¿Cuál es la función de una preposición?", points=5, quiz_id=quiz_lengua_sin_sala_3.id)


        session.add_all([ql1, ql2, qc1, qc2, qd1, qd2, qi1, qi2, qlss1_1, qlss1_2, qlss2_1, qlss2_2, qlss3_1])
        session.commit()
        for qn in [ql1, ql2, qc1, qc2, qd1, qd2, qi1, qi2, qlss1_1, qlss1_2, qlss2_1, qlss2_2, qlss3_1]: session.refresh(qn)
        ...
        # Lengua sin sala (3)
        session.add_all([
            Option(text="Unen palabras y establecen relaciones entre ellas", is_correct=True, question_id=qlss3_1.id),
            Option(text="Explican el significado de las palabras", is_correct=False, question_id=qlss3_1.id),
            Option(text="Modifican el valor de las palabras", is_correct=False, question_id=qlss3_1.id)
        ])
        ...
        for q_f in qs_fin:
            ...
            for idx, p in enumerate(fin_parts):
                is_ok = idx >= 2 
                session.add(Answer(points_earned=q_f.points if is_ok else 0, was_correct=is_ok, participant_id=p.id, room_id=r_finished.id, question_id=q_f.id, option_id=correct.id if is_ok else wrong.id))
