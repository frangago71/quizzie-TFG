...
const FinalScreen: React.FC<Props> = ({
    ...
    const fetchStats = async () => {
      try {
        const res = await api.get(
          `/stage/rooms/${roomId}/participants/${participantId}/stats`,
        );
        setStats(res.data);
      } catch (err) {
  ...
  return (
    <div
      className="live-room-wrapper podium-screen-container animate-fade-in final-screen-container"
      style={{ justifyContent: isHost ? "flex-start" : "center" }}
    >
      <div className="podium-titles">
        <h1 className="podium-main-title final-screen-title">
          ...
          {stats ? (
            <div className="final-stats-card">
              ...
              <p className="final-score-text">
                Has respondido{" "}
                <strong className="final-score-highlight">
                  {stats.correct_answers}
                </strong>{" "}
                correctas sobre {stats.total_questions} en total.
              </p>
            </div>
          ) : (
          ...
          {showExitModal && (
            <div className="modal-overlay">
              <div className="modal-content warning-modal">
                ...
                <p>
                  Tu nota aún no ha sido verificada por el profesor. Si sales
                  ahora, es posible que no se guarde correctamente.
                </p>
          ...
          <div className="final-actions">
            <div className="final-info-card">
              ...
              <h3 className="final-info-title">
                {status === "verifying"
                  ? "Fase de Verificación Activa"
                  : "Fase de verificación finalizada"}
              </h3>
              <p className="final-info-text">
                {status === "verifying"
              ...
              >
                ...
                <button
                  ...
                  onClick={async () => {
                    if (
                      window.confirm(
                        "¿Seguro que quieres cerrar la fase de verificación? Los alumnos ya no podrán validar sus notas.",
                      )
                    ) {
                      try {
              ...
              onScan={async (decodedText) => {
                ...
                try {
                  ...
                  const res = await api.post(
                    `/stage/rooms/${roomId}/verify-participant`,
                    {
                  ...
                  if (error.response && error.response.status === 409) {
                    toast.warning(
                      `El resultado de ${qrData?.nickname || "este alumno"} ya ha sido verificado anteriormente.`,
                    );
                  } else {
                    toast.error(
                      "Error al validar el código. Asegúrate de que sea un QR de Quizzie válido.",
                    );
                  }
                } finally {
