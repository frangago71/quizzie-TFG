...
const EditQuiz: React.FC = () => {
  ...
  const toggleOptionDeleted = (qIndex: number, oIndex: number) => {
    ...
    if (!isDeleted) {
      const activeOptions = newQuestions[qIndex].options.filter(
        (o) => !o._deleted,
      );
      if (activeOptions.length <= 2) {
        toast.warning(
          "No puedes borrar esta opción. Cada pregunta debe tener al menos dos opciones.",
        );
        return;
      }
      if (newQuestions[qIndex].options[oIndex].is_correct) {
        toast.warning(
          "No puedes borrar la opción correcta. Selecciona otra como correcta primero.",
        );
        return;
      }
    }
  ...
  const toggleQuestionDeleted = (index: number) => {
    ...
    if (!isDeleted) {
      ...
      if (activeQuestions.length <= 1) {
        toast.warning(
          "No puedes borrar la última pregunta. El cuestionario debe tener al menos una.",
        );
        return;
      }
    }
  ...
  const confirmSubmit = async () => {
    ...
    for (const { q, idx } of activeQuestions) {
      ...
      for (let oi = 0; oi < activeOptions.length; oi++) {
        if (!activeOptions[oi].text.trim()) {
          toast.warning(
            `La opción ${oi + 1} de la pregunta ${idx + 1} está en blanco.`,
          );
          setCurrentIndex(idx);
          setIsModalOpen(false);
          return;
    ...
    try {
      ...
      toast.error(
        err.response?.data?.detail || "Error al actualizar cuestionario",
      );
      setIsModalOpen(false);
    }
  };
  ...
  const isNextDisabled =
    isCurrentQuestionBlank || currentIndex === quiz.questions.length - 1;
      ...
      >
        ...
        <div className="quiz-main-layout">
          ...
          <div
            ...
            style={{
              opacity: currentQ._deleted ? 0.6 : 1,
              backgroundColor: currentQ._deleted ? "#f0f0f0" : "var(--card-bg)",
            }}
          >
            {quiz.questions.length > 1 && (
              <button
                ...
                onClick={() => toggleQuestionDeleted(currentIndex)}
                tabIndex={-1}
              >
                {currentQ._deleted ? "↶" : "✕"}
              </button>
              ...
              {currentQ.options.map((o, oIndex) => (
                <div
                  ...
                  style={{
                    ...
                    backgroundColor: o._deleted ? "#e0e0e0" : "transparent",
                  }}
                >
                  <input
                    type="radio"
                    ...
                    onChange={() =>
                      handleCorrectOptionChange(currentIndex, oIndex)
                    }
                    tabIndex={-1}
                  ...
                  {currentQ.options.length > 2 && !currentQ._deleted && (
                    <button
                      ...
                      tabIndex={-1}
                      onClick={() => toggleOptionDeleted(currentIndex, oIndex)}
                    >
                      {o._deleted ? "↶" : "✕"}
                    </button>
                  )}
        ...
        <div className="modal-overlay" onClick={() => setIsModalOpen(false)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <p>
                ¿Estás seguro de que quieres guardar los cambios? Esto
                actualizará el cuestionario y aplicará los borrados.
              </p>
            </div>
            <div
