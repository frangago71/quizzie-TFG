...
MAX_HTTP_P95_MS = 200              # Latencia P95 HTTP REST <= 200ms
MAX_AUTH_P95_MS = 500              # Latencia P95 Autenticación (Bcrypt Hashing) <= 500ms
MAX_WS_CONNECT_P95_MS = 200        # Latencia P95 WS Connect <= 200ms
...
def evaluate_thresholds(csv_path="tests/performance/benchmark_results_stats.csv"):
    ...
    cumplen los umbrales fijos de calidad de servicio (SLA/QA).
    """
    ...
    try:
        with open(csv_path, mode="r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                p95_val = float(p95_str) if p95_str and p95_str != "N/A" else 0.0
                ...
                # 3. Evaluación de Latencia P95 WebSocket Connect
                elif req_type == "WebSocket" and name == "WS Connect":
                    passed = p95_val <= MAX_WS_CONNECT_P95_MS
                    if not passed:
                        all_passed = False
                    evaluations.append({
                        "metric": "WebSocket Connect",
                        "actual": f"{p95_val:.0f} ms",
                        "threshold": f"<= {MAX_WS_CONNECT_P95_MS} ms",
                        "passed": passed
                    })
    ...
    print("\n==========================================================================")
    print(" === INFORME DE EVALUACION DE UMBRALES DE RENDIMIENTO ===")
    print("==========================================================================")
    print(f" {'Metrica / Endpoint':<35} | {'Valor P95':<12} | {'Umbral':<12} | {'Estado'}")
    print("--------------------------------------------------------------------------")
